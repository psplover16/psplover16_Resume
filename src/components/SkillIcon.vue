...
<template>
  <div class="skill-icon-wrapper">
    ...
    <span 
      v-else
      :class="['skill-icon-text', sizeClasses]"
      :title="skillName"
    >
      {{ textIcon }}
    </span>
...
const iconSrc = computed(() => {
  ...
  if (iconFile) {
    try {
      // 使用我們下載的圖標
      return new URL(`../assets/img/icons/${iconFile}.svg`, import.meta.url).href;
    } catch (error) {
...
const textIcon = computed(() => {
  return textIconMapping[props.skillName] || 
         props.skillName.charAt(0).toUpperCase() + 
         (props.skillName.split(/[\s.]+/)[1]?.charAt(0)?.toUpperCase() || '');
});
